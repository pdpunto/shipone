...
test("warning del proyecto activo ofrece editar next action", async () => {
  const originalLoad = Module._load;
  ...
  try {
    ...
    const nodes = await renderer.getRootNodes(false);
    const warningNode = nodes[1];

    assert.equal(warningNode.command.command, "shipone.editNextAction");
    assert.deepEqual(warningNode.command.arguments, ["p1"]);
  } finally {
    Module._load = originalLoad;
  }
});
