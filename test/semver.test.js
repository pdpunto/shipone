...
test("semver current devuelve la version del paquete", () => {
  assert.equal(runSemver(["current"]), "0.0.4");
});

test("semver tag devuelve la release tag", () => {
  assert.equal(runSemver(["tag"]), "v0.0.4");
});

test("semver validate-tag acepta el tag actual", () => {
  assert.equal(runSemver(["validate-tag", "v0.0.4"]), "v0.0.4");
});

test("semver beta-tag genera un prerelease", () => {
  assert.equal(runSemver(["create-beta-tag", "2"]), "v0.0.4-beta.2");
});

test("semver validate-beta-tag acepta un prerelease", () => {
  assert.equal(runSemver(["validate-beta-tag", "v0.0.4-beta.1"]), "v0.0.4-beta.1");
});
